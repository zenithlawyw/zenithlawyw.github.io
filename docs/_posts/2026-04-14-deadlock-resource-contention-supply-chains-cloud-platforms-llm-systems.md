...
Four conditions. All four present simultaneously, and the system stops. Permanently. The [Coffman conditions](<https://en.wikipedia.org/wiki/Deadlock_(computer_science)#Conditions>): mutual exclusion, hold and wait, no preemption, circular wait. These are the necessary and sufficient skeleton of [deadlock](<https://en.wikipedia.org/wiki/Deadlock_(computer_science)>), and they do not care whether the resources in question are memory pages, npm credentials, cloud billing regions, or GPU inference slots.
