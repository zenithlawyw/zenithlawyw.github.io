...
Can an attribution be checked? The paper answers with an unambiguous no for standard black-box models. {% include references/cite.html key="bhalla2023verifiable" %} Verification is a weaker requirement than faithfulness: you do not need the attribution to perfectly reflect the model's reasoning, only the ability to detect when it does not. The paper proves even that weaker requirement is impossible. The root cause: masking features produces out-of-distribution (OOD) inputs, and the model's response to OOD inputs need not correspond to its behaviour on the training distribution. You cannot distinguish between an incorrect attribution and a correct one evaluated on an unreliable input.
